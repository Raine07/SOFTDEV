...
class Common_model extends CI_Model {
    ...
    //get userinfo by id
    public function get_userinfo_by_slug($slug)
    {
        return $this->db->select('*')->from('users')->where('id',$slug)->get()->row();
    }
    ...
    // get all users
    function get_all_rated($total, $limit, $offset)
    {
        $this->db->select('u.*');
        $this->db->from('users as u');
        $this->db->where('role', 'user');
        $this->db->where('resilience_rating !=' , 'None');
